...
const Done = ({columns,setColumns}) => {
  ...
  const [doneTask, setDoneTask] = useLocalStorage("DoneTask", []);
  ...
  const AddDone = () => {
    ...
    setDoneTask((prev) => [
      ...prev,
      { id: newId, doneTask: "", priority: "", isEditing: true },
    ]);
    reset();
  };
  ...
  const handleDelete = (id) => {
    setDoneTask((prev) => prev.filter((item) => item.id !== id));
    handleClose();
  };

  const handleEdit = (id) => {
    const updatedRows = [...doneTask];
    const rowIndex = updatedRows.findIndex((row) => row.id === id);

    if (rowIndex !== -1) {
      updatedRows[rowIndex].isEditing = true;
      setDoneTask(updatedRows);

      const currentRow = updatedRows[rowIndex];
      setValue("doneTask", currentRow?.doneTask);
